...
- **터미널 우선 실행** - 파일을 바로 열거나, 현재 디렉터리를 스캔하거나,
  디렉터리를 넘겨 `fzf`로 일치하는 Markdown 파일을 고를 수 있습니다.
- **전용 미리보기 창** - Electron이 편집기 분할 대신 독립적인 데스크톱 창을
  제공합니다.
- **저장 시 라이브 리로드** - 대상 파일이 변경되면 미리보기가 업데이트됩니다.
- **Mermaid 기본 포함** - 별도 브라우저 설정 없이 fenced `mermaid` 블록이
  다이어그램으로 렌더링됩니다.
- **읽기 좋은 기본값, 로컬 설정** - 작은 TOML 파일로 테마, 폰트 스택, 폰트
  크기, 창 크기를 설정할 수 있습니다.
- **보수적인 렌더러 경계** - 렌더러에는 제한된 API만 노출되고, 외부 링크는
  미리보기 밖에서 열리며, 지원하지 않는 원시 HTML은 이스케이프 처리됩니다.
...
- `[^1]` 및 `^[인라인]` 문법을 통한 각주
- `mermaid` 펜스드 코드 블록
- 파일 기반 미리보기에서 Markdown 파일 경로를 기준으로 상대 이미지와 링크 해석
- Ctrl+F (macOS에서는 Cmd+F)로 미리보기 내에서 검색
...
추후 확장 예정인 기능으로는 파일 글로브 옵션과 더 풍부한 창 재사용 동작이
있습니다. 상대 경로 asset 해석은 파일 기반 미리보기에서 지원하며, stdin 입력은
여전히 파일 시스템 기준 경로가 없습니다.
