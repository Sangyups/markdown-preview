...
const gfmAlertLabels: Record<GfmAlertKind, string> = {
    caution: "Caution",
    important: "Important",
    note: "Note",
    tip: "Tip",
...
const gfmAlertMarkerPattern =
    /^\[!(NOTE|TIP|IMPORTANT|WARNING|CAUTION)\][\t ]*$/;

const buildOcticon = (name: string, path: string) =>
    `<svg class="octicon octicon-${name}" viewBox="0 0 16 16" width="16" height="16" aria-hidden="true"><path d="${path}"></path></svg>`;

const gfmAlertOcticons: Record<GfmAlertKind, string> = {
    caution: buildOcticon(
        "stop",
...
markdown.core.ruler.after("inline", "gfm_alerts", (state: CoreRulerState) => {
    ...
    for (let index = 0; index < tokens.length; index += 1) {
        ...
        const markerMatch = firstChild.content.match(gfmAlertMarkerPattern);
        ...
        const alertKind = markerMatch[1].toLowerCase() as GfmAlertKind;

        blockquoteOpen.tag = "div";
        tokens[closeIndex].tag = "div";
        blockquoteOpen.attrJoin(
            "class",
            `markdown-alert markdown-alert-${alertKind}`
        );

        const titleToken = new state.Token("gfm_alert_title", "", 0);
        titleToken.level = paragraphOpen.level;
        titleToken.meta = { gfmAlertKind: alertKind };

        const secondChild = children[1];
        const hasBodyOnSameParagraph =
            secondChild?.type === "softbreak" && children.length > 2;

        if (hasBodyOnSameParagraph) {
            children.splice(0, 2);
            tokens.splice(index + 1, 0, titleToken);
            continue;
        }

        tokens.splice(index + 1, 3, titleToken);
    }
});

markdown.renderer.rules.gfm_alert_title = (tokens, index) => {
    const kind = tokens[index].meta?.gfmAlertKind as GfmAlertKind | undefined;

    if (!kind) {
        return "";
    }

    return `<p class="markdown-alert-title">${gfmAlertOcticons[kind]}${gfmAlertLabels[kind]}</p>\n`;
};
